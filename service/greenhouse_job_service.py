...
class GreenhouseJobService:
    """Service to import jobs from Greenhouse API and manage an outbox repository."""
    ...
    def _company_from_domain(domain: str) -> str:
        """Extract company name from domain by removing .com and other prefixes."""
    ...
    def _fetch_jobs_for_company(self, company_name: str) -> Optional[Dict[str, Any]]:
        """Fetch jobs from Greenhouse API with retries."""
    ...
    def import_from_compressed(self, compressed_json_path: str) -> None:
        """Load compressed JSON, fetch jobs from Greenhouse, and save to repository."""
    ...
    def flush_outbox(self, publisher: Callable[[JobEntity], bool]) -> None:
        """Flush CREATED items using the provided publisher callable."""
...
def example_publisher(job: JobEntity) -> bool:
    """Example publisher (replace with real publish logic)."""
