...
class RipplingJobPlatformPollingService(JobPlatformPollingService):
    def __init__(self):
        ...
        self.process_job_data = self.async_concurrency_controller.limit_concurrency(self.process_job_data) # wrapped in concurrency controller
    ...
    async def process_job_data(self, job_data: dict, job_company_id: int) -> JobCreationRequest | None:
        try:
            ...
            script = soup.find("script", {"id": "__NEXT_DATA__"}).text.strip()
            val_json = json.loads(script)
            job_detail = val_json["props"]["pageProps"]["apiData"]["jobPost"]

            job_description = f"job location data : {job_detail.get('workLocations', None)};" + f"department data : {job_detail.get('department', None)};"
            #TODO: need to do processing on job_content
            job_description = job_description + f"job content data : {job_detail.get('description', None)}"
