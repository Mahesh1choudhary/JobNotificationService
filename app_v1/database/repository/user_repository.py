...
class UserRepository():
    ...
    async def save_user(self, user_creation_request:UserCreationRequest):
        query = f"""
            INSERT INTO {DatabaseTables.USER_TABLE.value} (user_name, user_email, user_telegram_user_name)
            VALUES ($1, $2, $3)
            ON CONFLICT (user_name) DO UPDATE 
            SET user_email = EXCLUDED.user_email,
                updated_at = NOW()
            RETURNING user_id, user_name, user_email, user_telegram_user_name, created_at, updated_at
        """
        try:
            row = await self._database_client.fetchrow(query, user_creation_request.user_name, user_creation_request.user_email, user_creation_request.user_telegram_user_name)
            if not row:
    ...
    async def find_all_by_user_ids(self, input_user_ids: list[int]) -> list[User]:
        query = f"""
            SELECT * FROM {DatabaseTables.USER_TABLE.value}
            WHERE user_id = ANY($1)
        """

        try:
            rows = await self._database_client.fetch(query, input_user_ids)
            return [User(**dict(row)) for row in rows]
        except Exception:
            logger.error(f"Database Error in find_all_by_user_ids with user_ids: {input_user_ids}", exc_info=True)
            raise
